    ...
    <form action="quizSubmitController.jsp?database=<%=dbName %>" method="post">
        ...
        for (Quiz q : list) { %>
            <div style="margin-bottom: 20px; padding: 10px; border: 1px solid gray;">
                ...
                <label><input type="radio" name="name<%= q.getSrno() %>" value="<%= q.getOpt1() %>"> <%= q.getOpt1() %></label><br>
                <label><input type="radio" name="name<%= q.getSrno() %>" value="<%= q.getOpt2() %>"> <%= q.getOpt2() %></label><br>
                <label><input type="radio" name="name<%= q.getSrno() %>" value="<%= q.getOpt3() %>"> <%= q.getOpt3() %></label><br>
                <label><input type="radio" name="name<%= q.getSrno() %>" value="<%= q.getOpt4() %>"> <%= q.getOpt4() %></label><br>
                <input type="hidden" name="correctAnswer<%= q.getSrno() %>" value="<%= q.getCorrectopt() %>">
            </div>
        <% } %>
